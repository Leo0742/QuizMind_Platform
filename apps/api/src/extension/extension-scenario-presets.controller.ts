...
export class ExtensionScenarioPresetsController {
  constructor(@Inject(AuthService) private readonly auth: AuthService,@Inject(ExtensionControlService) private readonly extensionControl: ExtensionControlService,@Inject(ExtensionScenarioPresetsService) private readonly svc: ExtensionScenarioPresetsService) {}
  ...
  @Get('catalog') async catalog(@Query() query:any){ return this.svc.listCatalog(query); }
  ...
  @Patch(':slug') async patch(@Param('slug') slug:string,@Headers('authorization') a:string|undefined,@Body() body:any){ return this.svc.updateMetadata(await this.requireSession(a, '/extension/scenario-presets/:slug'), slug, body); }
  ...
  private async requireSession(a:string|undefined, endpoint: string){ const t=parseBearerToken(a); if(!t) throw new UnauthorizedException('Missing Authorization bearer token.'); try { return await this.auth.getCurrentSession(t); } catch (error) { if (!(error instanceof UnauthorizedException)) throw error; } try { const installationSession = await this.extensionControl.resolveInstallationSession(t, { endpoint }); return buildInstallationRuntimeSession(installationSession);} catch { throw new UnauthorizedException('Session expired. Reconnect your account.'); } }
  private async trySession(a:string|undefined, endpoint: string){ const t=parseBearerToken(a); if(!t) return null; try { return await this.auth.getCurrentSession(t); } catch (error) { if (!(error instanceof UnauthorizedException)) return null; } try { const installationSession = await this.extensionControl.resolveInstallationSession(t, { endpoint }); return buildInstallationRuntimeSession(installationSession);} catch { return null; } }
}
